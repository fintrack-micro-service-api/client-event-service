pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'fintrack-client-event-service'
        CONTAINER_NAME = 'fintrack-client-event-service-container'
    ...
    stages {
        ...
        //                     -Dsonar.projectKey=fintrack-client-event-service \
        //                     -Dsonar.sources=src \
        ...
        stage('Trigger ManifestUpdate') {
            steps {
                script {
                    try {
                        build job: 'client-event-service-pipeline-2', parameters: [string(name: 'DOCKERTAG', value: env.BUILD_NUMBER)]
                    } catch (Exception e) {
                        currentBuild.result = 'FAILURE'
